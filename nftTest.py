...
def generate_trait_set_from_config(attribute_map):
    
    trait_set = []
    trait_paths = []
    # attribute_map = [2,1,1,1,1,1,1,0]
...
def generate_images(edition, count, attribute_map, drop_dup=True):
    ...
    op_path = os.path.join('static/output', 'edition ' + str(edition), 'images')
    ...
    for n in progressbar(range(count)):
        ...
        trait_sets, trait_paths = generate_trait_set_from_config(attribute_map)

        # print('trait_sets: ', trait_sets)
        # print('trait_paths: ', trait_paths)

        # Generate the actual image
        output_filename = os.path.join(op_path, image_name)
        image_file_name = generate_single_image(trait_paths, output_filename)
...
#     rt = generate_images(edition_name, 1)
